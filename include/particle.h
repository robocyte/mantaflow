...
{
public:
	ParticleSystem(FluidSolver* parent) 
        : ParticleBase(parent)
        , mDeletes(0)
        , mDeleteChunk(0)
    {}
	...
	void setPos(int idx, const Vec3& pos) { DEBUG_ONLY(checkPartIndex(idx)); mData[idx].pos = pos; }
	Vec3 getPos(int idx) { DEBUG_ONLY(checkPartIndex(idx)); return mData[idx].pos; }
	//! copy all positions into pdata vec3 field
	void getPosPdata(ParticleDataImpl<Vec3>& target);
	void setPosPdata(ParticleDataImpl<Vec3>& source);
	...
	void advectInGrid(FlagGrid& flags, MACGrid& vel, int integrationMode, bool deleteInObstacle=true, bool stopInObstacle=true );
...
public:
	...
	std::vector<BasicParticleData>& getData() { return mData; }
 	void printParts(int start=-1, int stop=-1, bool printIndex=false);
};
...
{
public:
	ParticleIndexSystem(FluidSolver* parent)
...
{
public:
	ConnectedParticleSystem(FluidSolver* parent)
...
public:
	...
	virtual ParticleDataBase*   clone()                         = 0;
	virtual PdataType           getType() const                 = 0; 
	virtual void                resize(int size)                = 0;
	virtual void                copyValueSlow(int from, int to) = 0;
...
public:
	...
	// interface (similar to grid data)
	void setConst(T s);
	...
	void printPdata(int start = -1, int stop = -1, bool printIndex = false);
...
template <class S>
struct GridAdvectKernel : public KernelBase
{
    GridAdvectKernel(std::vector<S>& p, const MACGrid& vel, const FlagGrid& flags, Real dt, bool deleteInObstacle, bool stopInObstacle)
        : KernelBase(p.size())
        , p(p)
        , vel(vel)
        , flags(flags)
    ...
    inline void op(int idx, std::vector<S>& p, const MACGrid& vel, const FlagGrid& flags, Real dt, bool deleteInObstacle, bool stopInObstacle, std::vector<Vec3> & u)
    {
    ...
    inline operator std::vector<Vec3> () { return u; }
    inline std::vector<Vec3>  & getRet() { return u; }
    inline std::vector<S>& getArg0() { return p; }
    typedef std::vector<S> type0;
    inline const MACGrid& getArg1() { return vel; }
    typedef MACGrid type1;
    inline const FlagGrid& getArg2() { return flags; }
    typedef FlagGrid type2;
    inline Real& getArg3() { return dt; }
    typedef Real type3;
    inline bool& getArg4() { return deleteInObstacle; }
    typedef bool type4;
    inline bool& getArg5() { return stopInObstacle; }
    typedef bool type5;
...
#pragma omp for
            for (int i=0; i < _sz; i++)
                op(i,p,vel,flags,dt,deleteInObstacle,stopInObstacle,u);
        }
    }
...
template <class S>
struct KnDeleteInObstacle : public KernelBase
{
    ...
    inline void op(int idx, std::vector<S>& p, const FlagGrid& flags)
    {
...
template <class S>
struct KnClampPositions : public KernelBase
{
    KnClampPositions(std::vector<S>& p, const FlagGrid& flags, ParticleDataImpl<Vec3> *posOld = NULL, bool stopInObstacle=true)
        : KernelBase(p.size())
        , p(p)
        , flags(flags)
        , posOld(posOld)
    ...
    inline void op(int idx, std::vector<S>& p, const FlagGrid& flags, ParticleDataImpl<Vec3> *posOld = NULL, bool stopInObstacle=true)
    {
	    if (p[idx].flag & ParticleBase::PDELETE) return;
...
#pragma omp for
            for (int i=0; i < _sz; i++)
                op(i,p,flags,posOld,stopInObstacle);
        }
    }
...
template <class S>
struct KnProjectParticles : public KernelBase
{
    ...
    inline void op(int idx, ParticleSystem<S>& part, Grid<Vec3>& gradient)
    {
