...
public:
    ...
    void advectInGrid(FlagGrid& flaggrid, MACGrid& vel, int integrationMode);
    void scale(Vec3 s);
    void offset(Vec3 o);

	void computeLevelset(LevelsetGrid& levelset, Real sigma, Real cutoff = -1.0);

	//! map mesh to grid with sdf
    template<typename T>
	void applyToGrid(GridBase* grid, T value, Real cutoff = -1.0, FlagGrid* respectFlags = nullptr)
    {
	    FluidSolver dummy(grid->getSize());
	    LevelsetGrid mesh_sdf(&dummy, false);
	    meshSDF(*this, mesh_sdf, 2.0, cutoff);

        ApplyMeshToGrid<T>(Grid, mesh_sdf, value, respectFlags)
	}
...
template <class T>
struct ApplyMeshToGrid : public KernelBase
{
    ...
    inline Grid<Real> & getArg1() { return sdf; }
...
#pragma omp for
                for (int j=0; j < _maxY; j++)
                    for (int i=0; i < _maxX; i++) op(i,j,k,grid,sdf,value,respectFlags);
            }
        }
    }
